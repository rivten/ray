...
v2i operator*(s32 Lambda, v2i A)
{
    v2i Result = V2i(Lambda * A.x, Lambda * A.y);
    return(Result);
}

v2i operator/(v2i A, s32 Lambda)
{
    Assert(Lambda != 0.0f);
    return((1.0f / Lambda) * A);
}
...
float Dot(v2i A, v2i B)
{
    return(A.x * B.x + A.y * B.y);
}

float DotClamp(v2i A, v2i B)
{
	return(Maxf(0.0f, Dot(A, B)));
}

float LengthSqr(v2i A)
{
    return(Dot(A, A));
}

void Normalize(v2i* A)
{
    *A = (*A) / sqrt(LengthSqr(*A));
}

v2i Normalized(v2i A)
{
    v2i Result;
    Result = A / sqrt(LengthSqr(A));
    return(Result);
...
v2i Lerp(v2i A, s32 t, v2i B)
{
    return((1.0f - t) * A + t * B);
}

v2i Hadamard(v2i A, v2i B)
{
    v2i Result;
    Result.x = A.x * B.x;
    Result.y = A.y * B.y;
...
v2 V2(v2i v)
{
	v2 Result = V2((float)v.x, (float)v.y);
	return(Result);
}
